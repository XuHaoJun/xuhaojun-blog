...
export function useCopyActions({ messages, conversationLogId }: UseCopyActionsOptions) {
  ...
  const copyCurrentMessage = async (index: number, override?: string) => {
    ...
    if (content) {
      try {
        await writeToClipboardWithFocus(content);
        toast.success(`已複製訊息內容 (${content.length} 字)`);
      } catch (error) {
        console.error("Copy failed:", error);
        toast.error("複製失敗，請確保頁面已獲得焦點");
      }
    }
  };
  ...
  const copyOriginal = async (index: number, override?: string) => {
    ...
    if (formattedContext) {
      try {
        await writeToClipboardWithFocus(formattedContext);
  ...
  const handleCompressedSubmit = async (limit: number) => {
    ...
    try {
      ...
      // 如果是 user，我們只壓縮之前的歷史 (targetIndex - 1)
      // 如果不是 user (如 assistant)，我們包含當前訊息一起壓縮 (targetIndex)
      const upTo = isUser ? targetIndex - 1 : targetIndex;
      ...
      if (extractedFacts || isUser) {
        ...
        if (!isUser) {
          ...
          const taskContent = contentOverride ?? targetMessage?.content;
        ...
        try {
          await writeToClipboardWithFocus(fullPackage);

          if (limitExceeded) {
            toast.warning(
              `已複製摘要內容 (${fullPackage.length} 字)，但已超過 ${limit} 字限制，已提供最佳壓縮版本`
            );
          } else {
